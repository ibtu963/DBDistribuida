...
public class ControladorView extends MouseAdapter implements ActionListener, Observer {
    ...
    public ControladorView(main vista, ComunicacionServer modeloNegocio, Ventas venta, insertar insertar){
        ...
        server = new SocketServidor(modeloNegocio.varUni.Puerto2);
    ...
    if(e.getSource()== venta.btnConfirmarVenta){
             ...
             t.start();
            //c = new SocketCliente(modeloNegocio.varUni.direccionIpSucu3,
            // modeloNegocio.varUni.Puerto3, modeloNegocio.PedidoFillMSJ(venta));
    ...
    private void actBtnTipoItem(String tipoItem) throws InterruptedException{
        switch(tipoItem){
            ...
            case "Comidas" -> {
                modeloNegocio.socketsGetComidas();
    ...
    private void actBtnEdit(){
        insertar.tfNombre.setText(modeloNegocio.Comida.nombre);
        insertar.tfPrecio.setText(modeloNegocio.Comida.precio);
        insertar.tfIngredientes.setText(modeloNegocio.Comida.ingredientes);       
        insertar.tfDetalles.setText(modeloNegocio.Comida.detalles);
        insertar.tfCantidadStock.setText(modeloNegocio.Comida.cantidad);
    ...
    private void  actBtnEnviarChat(){
        vista.txtApend.append("PC2: "+vista.tfChat.getText()+"\n");
       String msj = modeloNegocio.socketChat("PC2: "+vista.tfChat.getText()+"\n");
       SocketCliente c = new SocketCliente(modeloNegocio.varUni.direccionIpSucu1,
    ...
    private void actBtnPostres(){
        SocketCliente c = new SocketCliente(modeloNegocio.varUni.direccionIpSucu1,
             modeloNegocio.varUni.Puerto1,modeloNegocio.varUni.getPostresCode2);
    ...
    private void actBtnComidas(){
          modeloNegocio.auxComidas = modeloNegocio.comidasSocketMsj(modeloNegocio.socketsGetComidas());
    ...
    private void actBtnBebidas(){
     SocketCliente c = new SocketCliente(modeloNegocio.varUni.direccionIpSucu3,
        modeloNegocio.varUni.Puerto3,modeloNegocio.varUni.getBebidasCode2);
        modeloNegocio.varUni.waitBebidas = true;
        Thread t = new Thread(c);
        t.start();
     }
    
    private void actBtnInsertComida(){
        modeloNegocio.iniciar();
        auxComidaFill(); 
        modeloNegocio.ingresarComidas(modeloNegocio.Comida);
        JOptionPane.showMessageDialog(null,"Guardado con Éxito");
        insertar.setVisible(false);
        vista.setVisible(true);
    }
  
    private void actBtnUpdateComida(){
        auxComidaFill();
        modeloNegocio.updateComida(modeloNegocio.converMsjAComida( modeloNegocio.msjSocketUpComida(modeloNegocio.Comida)));  
     ...
     private void actBtnDeletePostre(){
        auxComidaFill();
        modeloNegocio.DeleteComida(modeloNegocio.converMsjAComida( modeloNegocio.msjSocketUpComida(modeloNegocio.Comida)));  
        JOptionPane.showMessageDialog(null,"Eliminado con Éxito");
        insertar.setVisible(false);
        vista.setVisible(true);
     }
   
    private void auxComidaFill(){
        modeloNegocio.Comida.nombre = insertar.tfNombre.getText();
        modeloNegocio.Comida.precio = insertar.tfPrecio.getText();
        modeloNegocio.Comida.ingredientes = insertar.tfIngredientes.getText();
        modeloNegocio.Comida.detalles = insertar.tfDetalles.getText();
        modeloNegocio.Comida.cantidad = insertar.tfCantidadStock.getText();
        }
    ...
    private void getPostreTabla(int fila){
        modeloNegocio.iniciar(); 
         ...
         modeloNegocio.Comida.id = vista.tabla.getValueAt(fila, 0).toString();
         modeloNegocio.Comida.nombre = vista.tabla.getValueAt(fila, 1).toString();
         modeloNegocio.Comida.precio = vista.tabla.getValueAt(fila, 2).toString();
         modeloNegocio.Comida.ingredientes = vista.tabla.getValueAt(fila, 3).toString();
         modeloNegocio.Comida.detalles = vista.tabla.getValueAt(fila, 4).toString();
         modeloNegocio.Comida.cantidad = vista.tabla.getValueAt(fila, 5).toString();
    ...
    public void update(Observable o, Object arg) {
        ...
        if(dato.equals(modeloNegocio.varUni.getComidasCode1)){
        ...
        if(dato.equals(modeloNegocio.varUni.getComidasCode3)){
                String a = modeloNegocio.socketsGetComidas();
                       //Sucursal 1
                       SocketCliente c = new SocketCliente(
